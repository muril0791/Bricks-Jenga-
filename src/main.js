...
import {
  buildTower, layoutView, setPhase, placeBet, cashOut, randomPick, newRound,
  backgroundGoldenTick, onCanvasMouseMove, onCanvasLeave, onCanvasClick
} from "./logic.js";
import { els, refreshHUD } from "./ui.js";
import { game } from "./state.js";
import { PHASE, CONFIG } from "./config.js";
...
function boot() {
  ...
  els.currencySel.addEventListener("change", () => {
    game.currency = els.currencySel.value;
    refreshHUD(game.phase);
  });
  els.langSel.addEventListener("change", () => {
    game.lang = els.langSel.value;
    refreshHUD(game.phase);
  });
  ...
  if (els.noCrashEl) {
    els.noCrashEl.checked = !!CONFIG.NO_CRASH;            
    els.noCrashEl.addEventListener("change", () => {      
      CONFIG.NO_CRASH = els.noCrashEl.checked;
    });
  }
